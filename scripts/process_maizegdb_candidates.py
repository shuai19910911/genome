#!/usr/bin/env python3
"""Download, validate, and archive MaizeGDB GFF3 annotation candidates."""
...
DEFAULT_CANDIDATES = LOCAL_REPORTS / "2026-06-07-maizegdb-candidates.tsv"
...
def main() -> int:
    ...
    selected_accessions = set(args.accession)
    jobs: list[tuple[dict[str, str], dict[str, str]]] = []
    for row in read_tsv(args.candidates):
        if row["has_gff3"] != "yes":
            continue
        if selected_accessions and row["assembly_accession"] not in selected_accessions:
            continue
        local = incomplete.get(row["assembly_accession"])
        if not local:
            continue
        report = Path(str(report_prefix(row["assembly_accession"], row["maizegdb_dir"])) + ".validation.md")
    ...
    for local, candidate in jobs:
        ...
        print(f"candidate\t{accession}\t{local['species']}\t{candidate['maizegdb_dir']}", flush=True)
        if args.dry_run:
            continue
