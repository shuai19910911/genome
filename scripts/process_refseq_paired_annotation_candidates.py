#!/usr/bin/env python3
"""Download, validate, and archive RefSeq annotation paired to local GenBank assemblies."""
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true", help="Re-run validation/finalization even if a report exists.")
    ...
    for row in read_tsv(args.candidates):
        ...
        if not local:
            continue
        report = Path(str(report_prefix(row["assembly_accession"], row["paired_refseq_accession"])) + ".validation.md")
    ...
    for local, candidate in jobs:
        ...
        paired_refseq = candidate["paired_refseq_accession"]
        print(f"candidate\t{accession}\t{paired_refseq}\t{local['species']}", flush=True)
        if args.dry_run:
            continue

        validation_dir = VALIDATION_DIR / accession
        gff3 = validation_dir / url_name(candidate["refseq_gff3_url"])
        gtf = validation_dir / url_name(candidate["refseq_gtf_url"]) if candidate["refseq_gtf_url"] else None
        download(candidate["refseq_gff3_url"], gff3)
        gzip_ok(gff3)
        if gtf is not None:
            download(candidate["refseq_gtf_url"], gtf)
            gzip_ok(gtf)
        ...
        if gtf is not None:
            validate_cmd.extend(["--gtf", str(gtf.relative_to(ROOT))])
        run(validate_cmd)
        ...
        run(
            [
                ...
                "--source-name",
                "NCBI RefSeq paired assembly",
                "--source-dir",
                source_dir(candidate["refseq_gff3_url"]),
                "--gff3-url",
                candidate["refseq_gff3_url"],
                "--gtf-url",
                candidate["refseq_gtf_url"] or "未提供",
                "--gtf-build-accession",
                paired_refseq,
                "--gtf-genome-version",
                candidate["paired_refseq_assembly_name"],
                "--validation-summary",
                validation_summary(report),
            ]
            + (["--gtf", str(gtf.relative_to(ROOT))] if gtf is not None else [])
        )
