...
- 当前状态来源: `docs/current-download-status.md`、`local_reports/completed-genome-index.tsv`、`local_reports/incomplete-genome-index.tsv`

## 当前结论

- NCBI 第一轮下载已经结束，没有下载进程和 aria2 活动任务。
- 计划条目一共 1906 个。
- 有 genome 和至少一种注释的目录有 254 个，其中 149 个来自原始完成清单，其余主要来自后续外部注释补全。
- 未完整目录有 1652 个；这些目录不是空目录，都已经有 genome 文件，主要缺少 GFF3/GTF 注释。
...
- 第一批: 从本地 `local_reports/incomplete-genome-index.tsv` 中按物种建立专项数据库候选表，先覆盖失败最多的作物。
