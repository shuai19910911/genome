...
- 更新时间: 2026-06-07 14:39:30
- 完成记录数: 149
- 失败记录数: 1755
- 已生成 assembly 目录数: 1906
- 已生成 README 的完成目录数: 191
- 本地数据总量: 902546542734 bytes (840.56 GiB)
- 下载管理进程数: 0
- aria2 活动下载数: 0
- 最近速度: 0.00 MiB/s
...
- 有 genome 和至少一种注释的目录: 196
- 只有 genome、缺少 GFF3/GTF 注释的目录: 1710
- 完整索引: `docs/completed-genome-index.tsv`
- 未完整索引: `docs/incomplete-genome-index.tsv`

## 外部注释补充

- 已将 40 个 Ensembl Plants 候选 GFF3/GTF 归档到本地 assembly 目录，其中本次检查新增 12 个大麦候选。
- 已归档: `Triticum_aestivum_GCA_949126075.2`、`Cucumis_melo_GCA_902497455.1`、`Citrullus_lanatus_GCA_000238415.2`、`Solanum_tuberosum_GCA_014189475.1`、`Malus_domestica_GCA_002114115.1`、13 个 `Oryza sativa` assembly，以及 22 个 `Hordeum vulgare` assembly。
- 本次新增 12 个大麦候选的 Ensembl GTF metadata accession/genome-version 与本地 assembly 一致，并已按染色体/contig 别名和长度验证通过。
...
- Ensembl Plants 候选扫描覆盖失败最多的前 12 个物种，发现 70 个同物种候选目录；目前已有 40 个候选完成坐标验证和归档。
