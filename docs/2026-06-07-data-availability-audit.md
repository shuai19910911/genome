...
本地作物基因组库当前包含 1906 个 assembly 目录，其中 258 个已有 genome 和至少一种注释文件，1648 个只有 genome、缺少 GFF3/GTF 注释。注释缺口不是下载中断造成的空目录问题，而主要是 GenBank GCA 条目在 NCBI 端缺少 GFF3/GTF 注释。

外部注释补全路线目前呈现明显分层：Ensembl Plants 是当前最高收益路线，已验证通过 98 个候选、失败 6 个；RefSeq paired assembly 路线小样本 3/3 通过；LegumeInfo 2 个候选中 1 个通过；Gramene 与 MaizeGDB 当前验证样本均未通过，不能作为直接归档路线。
...
| genome+注释完整目录 | 258 | `local_reports/completed-genome-index.tsv` |
| 只有 genome、缺少 GFF3/GTF | 1648 | `local_reports/incomplete-genome-index.tsv` |
...
| 路线 | 验证通过 | 验证失败 | 当前解释 |
|---|---:|---:|---|
| Ensembl Plants | 98 | 6 | 最高收益路线；必须继续做 accession、assembly、品种和长度级验证 |
...
- Ensembl Plants 当前已经归档 98 个候选，其中 Hordeum vulgare 贡献 76 个；Hordeum vulgare Ensembl assembly-name 候选目前已清空。
